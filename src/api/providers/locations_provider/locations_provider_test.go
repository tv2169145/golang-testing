...
func TestGetCountryRestClientError(t *testing.T) {
	...
	assert.EqualValues(t, "invalid restclient error to getting country AR", err.Message)
}
...
func TestGetCountryInvalidErrorInterface(t *testing.T) {
	...
	_ = rest.AddMockups(&rest.Mock{
		...
		HTTPMethod: http.MethodGet,
		RespHTTPCode: http.StatusNotFound,
		RespBody: `{"message": "invalid error interface when getting country AR", "error": "not_found", "status": 500, "cause": []}`,
	})
	country, err := GetCountry("AR")
	assert.Nil(t, country)
	assert.NotNil(t, err)
...
func TestGetCountryErrorResponseAndInvalidJsonBody(t *testing.T) {
	rest.FlushMockups()
	_ = rest.AddMockups(&rest.Mock{
		URL: "https://api.mercadolibre.com/countries/AR",
		HTTPMethod: http.MethodGet,
		RespHTTPCode: http.StatusNotFound,
		RespBody: `{"message": 123, "error": "not_found", "status": 500, "cause": []}`,
	})
	country, err := GetCountry("AR")
	assert.Nil(t, country)
	assert.NotNil(t, err)
